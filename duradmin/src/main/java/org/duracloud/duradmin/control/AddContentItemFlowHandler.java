...
public class AddContentItemFlowHandler extends AbstractFlowHandler {
    ...
    public String handleExecutionOutcome(FlowExecutionOutcome outcome,
        ...
        ContentItem contentItem = (ContentItem)outcome.getOutput().get(CONTENT_ITEM);
        
        String outcomeUrl = null;
        
        if (outcome.getId().equals(SUCCESS_OUTCOME)) {
            outcomeUrl = MessageFormat.format("contextRelative:/content.htm?spaceId={0}&contentId={1}", 
                                         space.getSpaceId(), contentItem.getContentId());
            outcomeUrl = MessageUtils.appendRedirectMessage(outcomeUrl, MessageUtils.createMessage("Successfully added content."), request);

        } else if(returnTo == null) {
            outcomeUrl = MessageFormat.format("contextRelative:/contents.htm?spaceId={0}", space.getSpaceId());
        }else{
            outcomeUrl = returnTo;
        }
