    ...
    protected Credential getRootCredential() {
        try {
            ...
            if(!credentialsFile.exists()){
                generateEncryptedCredentialsFile(encryptionUtil,
                        credentialsFile);
            }
    ...
    private String decryptValue(Properties props, EncryptionUtil encryptionUtil,
        ...
        if(value != null){
            try {
                ...
                String message = MessageFormat.format("Failed to decrypted value stored in key {0} in {1}", key, credentialsFile.getAbsolutePath());
                log.error(message);
                throw new RuntimeException(message);
            
            }
        }else{
            String message = MessageFormat.format("{0} is not present in credentials file {1}", key, credentialsFile.getAbsolutePath());
            log.error(message);
            throw new RuntimeException(message);
        }
    }
